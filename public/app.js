...
async function checkQrStatus(orderId){
  try{
    ...
    if(d.status==='rejected'){ stopPolling(); const msg='<div class="error"><b>❌ PAYMENT REJECTED</b><br>Submitted UTR / Transaction ID payment SMS se match nahi hua. Koi ID release nahi ki gayi.</div>'; $('payStatus').innerHTML=msg; $('utrMsg').innerHTML=msg; return; }if(d.status==='expired'){stopPolling();$('payStatus').innerHTML='<div class="error">QR expired. Please click Buy Now again to generate a new QR.</div>';return;}
