...
async function checkQrStatus(orderId){
  try{
    ...
    if(d.status==='rejected'){ stopPolling(); const msg='<div class="error"><b>❌ PAYMENT REJECTED</b><br>Aapne galat UTR / Transaction ID submit kiya hai, isliye payment request reject ho gayi.<br><b>Kripya sahi UTR submit karein aur payment karke dobara payment karein.</b><br><small>Payment rejected — ID release nahi hogi.</small></div>'; $('payStatus').innerHTML=msg; $('utrMsg').innerHTML=msg; return; }if(d.status==='expired'){stopPolling();$('payStatus').innerHTML='<div class="error">QR expired. Please click Buy Now again to generate a new QR.</div>';return;}
