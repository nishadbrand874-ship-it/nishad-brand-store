...
async function checkQrStatus(orderId){
  try{
    ...
    if(d.status==='rejected'){ stopPolling(); const msg='<div class="error"><b>❌ PAYMENT REJECTED</b><br>Your UTR / Transaction ID was rejected.<br><b>Kripya sahi UTR submit karein aur payment karke dobara payment karein.</b><br><small></small></div>'; $('payStatus').innerHTML=msg; $('utrMsg').innerHTML=msg; return; }if(d.status==='expired'){stopPolling();$('payStatus').innerHTML='<div class="error">QR expired. Please click Buy Now again to generate a new QR.</div>';return;}
